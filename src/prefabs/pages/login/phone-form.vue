<template>
    <i-form @submit="storeToken">
        ...
        <i-input ref="phoneInput" v-model="form.phone" v-model:error="error.phone" name="phone" type="tel"
            placeholder="请输入手机号" attr="手机号" :rules="[
                'required', 'phone',
            ]" theme="auth-page">
        ...
        <sms-input ref="smsInput" v-model="form.sms" v-model:error="error.sms" name="sms" type="text"
            :captcha-input="captchaInput" :phone-input="phoneInput" :phoneNumber="form.phone" v-model:mobile="form.phone"
            v-model:uuid="form.uuid" v-model:imgCode="form.imgCode" v-model:scene="form.scene" placeholder="请输入验证码"
            attr="验证码" :rules="[
                'required',
            ]" theme="auth-page">
...
const storeToken = () => {
    loading.value = true;
    let formData = {
        mobile: encrypt(form.value.phone),
        code: encrypt(form.value.sms),
    };
    axios.post('/auth/sms-login', formData).then(({ data }) => {
        if (data.code == 0) {
